...
def checkGames():
    ...
    games = requests.get(f"https://idle-game-api.crabada.com/public/idle/mines?user_address={ADDRESS}&page=1&status=open&limit=8", headers=HEADERS).json()["result"]["data"]
    for game in games:
        now = time.time()
