...
def getTeamsData(address):
    ...
    teamsData = requests.get(teamsUrl,headers=HEADERS).json()["result"]["data"]
    return teamsData 
...
def checkAttackableMineHistory(address):
    ...
    mineHistory = requests.get(mineHistoryUrl, headers=HEADERS).json()["result"]["data"]
...
def isMineAttacked(mineId):
    ...
    mine = requests.get(mineUrl, headers=HEADERS).json()["result"]
